...
public class OrdreGeologues extends OrdreProfessionnel {
    ...
    final String PATTERN_NUMERO_DE_PERMIS = "[0-9]{4}$";
    
    public OrdreGeologues(JSONObject declaration) throws ParseException {
        super(declaration);
    }
    
    @Override
    public String getPatternNumeroDePermis(){
        return "^" + declaration.getString("nom").toUpperCase().charAt(0) + declaration.getString("prenom").toUpperCase().charAt(0) + PATTERN_NUMERO_DE_PERMIS;
    }
    ...
    public Cycle creerCycle(String cycle) throws ParseException {
        switch (cycle) {
        case "2013-2016":
            return new Cycle("2013-06-01", "2016-06-01", 55.0);
